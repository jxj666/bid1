...
var common = {
    ...
    select_show: function(data) {
        ...
        show_l(list, main.data.s_channel, '#s_channel', 1, '渠道');
        show_l(list2, main.data.s_field, '#s_field', 2, '领域');
        show_l(list3, main.data.s_author, '#s_author', 3, '作者');
    ...
    skip: function(res) {
        ...
        var start = main.data.ajax1.start;
        var num = 20;
        var now = (start) / 20 + 1;
        var prev = now - 1;
        var next = now + 1;
        ...
        var html1 = `<div class="first_div"><span>共${total}条</span></div><div><span class="sum">每页20条</span></div><div class="prev_est"></div><div class="prev"></div><div class="page_num"><div class="num num1 ${prev<min&&"hide"}">${prev}</div><div class="num num2">${now}</div><div class="num num3 ${next>max&&"hide"}">${next}</div><div class="clear"></div></div><div class="last"></div><div class="last_est"></div><div><span class="intro">当前${now}/${max}页</span></div><div class="last_div"><span>转到第<input type="text" name="" class="arrow">页</span></div>`;
        $('.m_skip').html(html1);
    },
