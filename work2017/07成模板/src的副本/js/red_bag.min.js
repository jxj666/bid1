...
$($('.nav')[0]).text('红包').addClass('hongbao');
...
//拆大红包
$('.j_bag_btn').on('click', function() {
    $('.redbag').addClass('donhua')
    setTimeout(function() {
        $('.redbag').removeClass('donhua')
        var i = Math.random() * 9;
        if (i > 5) { $('.honbao_t').removeClass('invisible'); } else { $('.xiexie_t').removeClass('invisible'); }
        $('.j_bag_btn').addClass('redbag_off')

    }, 1000)


});
// $('.lucky-pop button').on('click', function() {
//     $('div.pop-bg').addClass('invisible');

//     location.reload();

// });
$('.honbao_t button').on('click', function() {
    $('div.pop-bg').addClass('invisible');

    location.href='detail.html?index=2';

});
$('.xiexie_t button').on('click', function() {
    $('div.pop-bg').addClass('invisible');
